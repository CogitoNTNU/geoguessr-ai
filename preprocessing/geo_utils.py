...
def haversine_np(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    a = (
        np.sin(delta[:, 1] / 2) ** 2
        + np.cos(x_rad[:, 1]) * np.cos(y_rad[:, 1]) * np.sin(delta[:, 0] / 2) ** 2
    )
...
def haversine(x: Tensor, y: Tensor) -> Tensor:
    ...
    delta = y_rad - x_rad
    a = (
        torch.sin(delta[:, 1] / 2) ** 2
        + torch.cos(x_rad[:, 1])
        * torch.cos(y_rad[:, 1])
        * torch.sin(delta[:, 0] / 2) ** 2
    )
...
def haversine_matrix_np(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    p = np.expand_dims(np.cos(x_rad[:, 1]), axis=1) * np.expand_dims(
        np.cos(y_rad[1, :]), axis=0
    )
...
# Inspired by https://github.com/kvenkman/ecef2lla
def cylindrical2geodetic(
    rho: Tensor, z: Tensor, a: Tensor, f: Tensor, device: torch.device
) -> Tensor:
    # Reshape
    zz = z.unsqueeze(1)
