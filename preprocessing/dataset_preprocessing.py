...
from shapely.errors import (
    ShapelyError,
)  # replaced pygeos(outdated and not in modern python)
import torch
import numpy as np
import pandas as pd
...
from config import GEOCELL_PATH, CLIP_MODEL
...
def change_labels_for_classification(dataset: DatasetDict) -> DatasetDict:
    ...
    train_labels = generate_cell_labels_vector(
        dataset["train"]["labels"], geocell_df["polygon"]
    )
    val_labels = generate_cell_labels_vector(
        dataset["val"]["labels"], geocell_df["polygon"]
    )
    test_labels = generate_cell_labels_vector(
        dataset["test"]["labels"], geocell_df["polygon"]
    )
...
def load_geocell_df(path: str = GEOCELL_PATH) -> gpd.GeoDataFrame:
    ...
    geocell_df = gpd.GeoDataFrame(
        geocell_df,
        geometry=gpd.points_from_xy(geocell_df.lng, geocell_df.lat),
        crs="EPSG:4326",
    )
...
def generate_cell_labels(
    point: Point, geocell_df: GeoDataFrame, one_hot: bool = True
) -> np.ndarray:
...
def generate_label_cells(
    example: Dict, geocell_df: gpd.GeoDataFrame = None, one_hot: bool = True
) -> Dict:
...
def extract_features(example: Dict, feature_extractor: AutoFeatureExtractor) -> Dict:
    ...
    """
    x = {}
    x["pixel_values"] = feature_extractor(images=example["image"], return_tensors="pt")[
        "pixel_values"
    ]
    if "image_2" in example:
        for i in range(2, 5):
            x[f"pixel_values_{i}"] = feature_extractor(
                images=example[f"image_{i}"], return_tensors="pt"
            )["pixel_values"]
...
def add_embeddings(example: Dict, index: int, emb_array: np.ndarray) -> np.ndarray:
    ...
    return {"embedding": emb_array[index]}
...
def preprocess(
    dataset: DatasetDict, geocell_path: str, embedder: Any = None
) -> DatasetDict:
    ...
    if embedder is not None:
        ...
        for split_name, split_dataset in dataset.items():
            ...
            dataset[split_name] = split_dataset.map(
                add_embeds, with_indices=True, keep_in_memory=False, num_proc=1
            )
        ...
        dataset = dataset.map(
            lambda x: extract_features(x, feature_extractor),
            batch_size=128,
            batched=True,
            num_proc=64,
        )
    ...
    dataset = dataset.map(
        lambda x: generate_label_cells(x, geocell_df, False),
        num_proc=8,
        keep_in_memory=False,
    )  # TODO: batch label creation
