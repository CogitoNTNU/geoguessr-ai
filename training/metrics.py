...
from config import DECAY_CONSTANT, SCALER_PATH, COUNTRY_PATH
...
def recover_regression_values(values: np.ndarray) -> np.ndarray:
...
def compute_geoguessr_metrics(results: np.ndarray) -> Dict:
    ...
    if labels_mt is not None:
        preds_mt = recover_regression_values(preds_mt)
